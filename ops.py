...
def synthesis_const_block(res, w_broadcasted, n_f, noise, noise_, sn=False):
    w0 = w_broadcasted[:, 0]
    w1 = w_broadcasted[:, 1]
    ...
    with tf.variable_scope('{:d}x{:d}'.format(res, res)):
        with tf.variable_scope('const_block'):
            ...
            x = apply_noise(x, noise) # B module
            x = apply_bias(x, lrmul=1.0)
        ...
        with tf.variable_scope('Conv'):
            ...
            x = apply_noise(x, noise_) # B module
            x = apply_bias(x, lrmul=1.0)
...
def synthesis_block(x, res, w_broadcasted, layer_index, n_f, noise, noise_, sn=False):
...
def Cal_style_loss(real_y, fake_y, Ms, style_model, Lambda=1):
    real_y = tf.expand_dims(tf.image.resize(real_y, [224, 224]), axis=0)
    fake_y = tf.expand_dims(tf.image.resize(fake_y, [224, 224]), axis=0)

    real_style = style_model(real_y)
    fake_style = style_model(fake_y)
...
def Cal_percep_loss(real_x, fake_x, Mp, percep_model, Lambda=1):
    ...
    fake_x = tf.expand_dims(tf.image.resize(fake_x, [224, 224]), axis=0)

    percep_loss = 0
    real_perceps = percep_model(real_x)
    fake_perceps = percep_model(fake_x)
    for real_percep, fake_percep in zip(real_perceps, fake_perceps):
        Mp_cur = cv2.resize(Mp, (real_percep.shape[1], real_percep.shape[2]))
